...
YEAR = 2025
DAILY_INDEX = pd.date_range(start=f"{YEAR}-01-01", end=f"{YEAR}-12-31", inclusive="both", freq="D")
HOURLY_INDEX = pd.date_range(start=f"{YEAR}-01-01", end=f"{YEAR+1}-01-01", inclusive="left", freq="h")    
...
def get_prices():
    ...
    df_prices = pd.DataFrame({
        "index": HOURLY_INDEX[:len(prices)],
        "price_euro_per_MWh": prices 
        }
    )

    df_prices.to_csv(ROOT / "prices.csv", index=False)
    print(f"DONE prices.")
...
def get_pv_production():
    ...
    power_dc = np.asarray(data["outputs"]["dc"]) * 1e-3 / capacity_placeholder_kW * capacity_true_MW
    ...
    # write
    df_pv.to_csv(ROOT / "pv.csv", index=False)
    print(f"DONE PV generation.")
