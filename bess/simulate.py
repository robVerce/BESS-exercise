...
def dispatch(
    ...
    for index, (index_original, price) in enumerate(elements_charging):
        ...
        power_charge_max_MW = min(
            power_charge_max_MW, 
            (bess_rated_capacity_true-total_energy_charged_MWh)
        )
...
def simulate_year(
    ...
    df_results = pd.DataFrame(
        ...
        data={
            ...
            "prices_electricity_euro_per_MWh": df_prices_electricity["price_euro_per_MWh"],
    ...
    for i in range(365):
        ...
        prices_electricity_day = df_results["prices_electricity_euro_per_MWh"].values[i_start:i_end]
        ...
        for j in range(24):
            ...
            power_bess_net_discharge_MW_raw = power_bess_charge_MW[j] + power_bess_discharge_MW[j]
            power_bess_net_discharge_MW = power_bess_charge_MW[j] + power_bess_discharge_MW[j]*bess_rated_efficiency_true
            state_battery_energy_kWh -= (power_bess_charge_MW[j]*bess_rated_efficiency_true +power_bess_discharge_MW[j])
            power_sold_net_MW = generation_pv_day[j] + power_bess_net_discharge_MW

            assert power_sold_net_MW <= grid_rated_power_MW
            assert -power_sold_net_MW <= grid_rated_power_MW

            df_results.at[index, "power_bess_net_discharge_raw_MW"] = power_bess_net_discharge_MW_raw
            df_results.at[index, "power_bess_net_discharge_MW"] = power_bess_net_discharge_MW
...
def postprocess(df, bess_rated_energy_MWh, bess_rated_power_MW):
    ...
    total_energy_discharged_MWh = df["power_bess_net_discharge_raw_MW"].clip(lower=0).sum()
...
def main(config):
    ...
    # load data
    df_prices_electricity = pd.read_csv(ROOT / "data" / "prices.csv", index_col="index", parse_dates=True)
    df_pv_generation = pd.read_csv(ROOT / "data"/ "pv.csv", index_col="index", parse_dates=True)
